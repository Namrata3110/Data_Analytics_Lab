...
barplot(table(dataset$Species, dataset$Petal.Length), main="Species and Petal Length", col=c(brewer.pal(n = 3, name = "Accent")))
...
boxplot(dataset$Sepal.Length~dataset$Species, main="Species vs. Sepal Length",col=rainbow(3))
#d) Create a scatter plot with Species along the Y axis and Petal Length along the X axis.

plot(x=dataset$Petal.Length,y=dataset$Species)